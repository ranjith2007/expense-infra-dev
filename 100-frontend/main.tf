...
resource "aws_autoscaling_group" "frontend" {
  ...
  # If instances are not healthy with in 15min, autoscaling will delete the instance
  timeouts {
    delete = "15m"
  }
...
resource "aws_lb_listener_rule" "frontend" {
  ...
  condition {
    host_header {
      values = ["expense-${var.environment}.${var.zone_name}"] #expense-dev.devops81s.site
    }
  }
}